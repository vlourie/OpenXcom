...
def silhouette_pairs(sh, thr=25.0, hue_thr=70.0):
    ...
    поворот) и поворот со своим светом укладывается в thr. Порог 25 взят не на глаз, а по
    замеру на 700 парах, где мы успели нарисовать ОБОИХ близнецов: до 25 вывод отличается
    от нарисованного на 7-9.6 из 255, и хуже 19 выходит у 0-5 процентов пар - при том, что
    два независимых заказа ОДНОЙ картинки расходятся на 19 сами по себе. После 25 - уже
    12.3 и 18 процентов, и такие дешевле нарисовать: их 104 из 6526.
    ...
    # Предохранитель на общий источник. В наборе бывают кадры, совпадающие побайтово
    # (ICEKING_RUINS: 70 и 76 - одна картинка). Их зеркала 77 и 78 при этом РАЗНЫЕ,
    # отличаются на 10.5 из 255. Вывести оба из общего источника значит сделать их
    # одинаковыми: рецепт даёт между ними 3.0, перенос цвета вытягивает лишь до 4.5 -
    # разница у них в фактуре, а её из общего источника не взять ничем. Такие пары
    # отдаём модели целиком, обе.
    def key(i):
        return hashlib.blake2b(arrs[i].tobytes(), digest_size=16).hexdigest()

    by_src = {}
    for n, (i, _j, _e) in enumerate(out):
        by_src.setdefault(key(i), []).append(n)
    bad = set()
    for _h, group in by_src.items():
        if len(group) > 1 and len({key(out[n][1]) for n in group}) > 1:
            bad.update(group)
    return [t for n, t in enumerate(out) if n not in bad]
