...
def collect(root, patterns, set_override):
    """{набор: {кадр: {"control": путь|None, "targets": [пути]}}}"""
    got = {}
    dirs = []
...
def run(args):
    ...
    got = collect(root, args.src, args.set_name)
    ...
    for set_name in sorted(got):
        ...
        for i in frames:
            ...
            if not flags or args.keep_bad:
                ...
                cap = args.caption.format(hint=hint or "terrain tile", kind=info["kind"],
                                          trigger=args.trigger, set=set_name)
            ...
            report.append({"set": set_name, "frame": i, "name": name, "src": src,
                           "kind": info["kind"], "hint": hint, "how": info["how"],
                           "flags": flags, **mt})
...
def write_report(args, report, counts):
    ...
    with open(path, "w", encoding=ENC, newline="") as f:
        f.write("набор\tкадр\tимя\tвид\tслед\tфактура\tкрупное оригинал\tкрупное клетка\t"
                "крупное отн\tтон\tзамечания\tкузница\tподсказка\tисходник\n")
        for r in report:
            f.write("%s\t%d\t%s\t%s\t%.3f\t%.2f\t%.1f\t%.1f\t%.2f\t%.0f\t%s\t%s\t%s\t%s\n"
                    % (r["set"], r["frame"], r["name"], r["kind"], r["iou"], r["det"],
    ...
    lines += ["", "## Дальше", "",
              ...
              "3. Подпись в `.txt` рядом с картинкой обязана совпадать с тем промптом,",
              "   которым потом будем генерить, иначе LoRA не включится.", ""]
    with open(os.path.join(args.out, "report.md"), "w", encoding=ENC) as f:
        f.write("\n".join(lines))

    # манифест: его читает tomllib, спецификация там лишняя (грабли R-001, исключение)
...
DEFAULT_CAPTION = ("{trigger}isometric terrain tile, {hint}, hand-painted high detail, "
                   "2:1 isometric diamond, flat top-down lighting, grey preview background")
