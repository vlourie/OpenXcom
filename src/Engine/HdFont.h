...
private:
	struct Key
	{
		...
		bool operator==(const Key &o) const { return c == o.c && px == o.px && cond == o.cond; }
	};
	struct KeyHash
	{
		size_t operator()(const Key &k) const { return (size_t)k.c * 0x9E3779B9u ^ ((size_t)k.px << 16) ^ ((size_t)k.cond << 28); }
	};
