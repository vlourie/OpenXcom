...
public:
	...
	static std::string calcStatStringWorker(UnitStats &currentStats, int rank, const std::vector<StatString*> &statStrings, bool psiStrengthEval, bool inTraining);
	/// Get the CurrentStats.
	static std::map<std::string, int> getCurrentStats(UnitStats &currentStats, int rank);
};
